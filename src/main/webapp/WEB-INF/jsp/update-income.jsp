	...
	<form:form method="post" modelAttribute="income">
		...
		<select id="sel-category">
		    ...
		    <c:forEach items="${categories}" var="category"> 
            	<option value="${category.getId()}" <c:if test="${category.getName() == income.getSubCategory().getCategory().getName()}"> selected="selected" </c:if> >${category.getName()}</option> 
            </c:forEach>
		</select>
		...
		<fieldset class="form-group">
			...
			<fmt:formatDate value="${income.getDateTime()}" pattern="yyyy/MM/dd HH:mm:ss" var="myDate" />
		...
		<button id="update-income" type="submit" class="btn btn-success">Zatwierdź</button>
